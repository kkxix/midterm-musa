...
park_distance <- function(dist_matrix_parks, k) {
  ...
  # Sort and take first k, then average   mean(as.numeric(sort(distances)[1:k]))
  })
  }
...
hosp_distance <- function(dist_matrix_hospitals, k) {
  apply(dist_matrix_hospitals, 1, function(distances) {# Sort and take first k, then average
...
#feature sets#### \# structural features structural_features \<-
...
#models#### \# structural model structural_model \<- lm(log_price \~
number_of_bedrooms + number_of_bathrooms + total_livable_area +
...
summary(structural_model) summary(census_model) summary(spatial_model)
